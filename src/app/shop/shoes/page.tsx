...
function ShoesPageContent() {
  ...
  const subcategoryFromPath = pathSegments[pathSegments.indexOf('shoes') + 1]
  ...
  // Get all shoes
  const allProducts = getProductsByCategory('Shoes')
  
  // Set up initial filters
  useEffect(() => {
    setFilter('category', 'Shoes')
    if (subcategory) {
  ...
  const breadcrumbItems = [
    ...
    { label: 'Shoes', href: '/shop/shoes', isActive: !subcategory }
  ]
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <h1 className="text-3xl font-serif font-semibold text-ink">
              {subcategory ? `${subcategory.charAt(0).toUpperCase() + subcategory.slice(1)} Shoes` : 'Shoes'}
            </h1>
          ...
          <p className="text-ink/70 text-lg max-w-2xl">
            {subcategory 
              ? `Discover our collection of ${subcategory} shoes, crafted for comfort and style.`
              : 'Step into luxury with our complete collection of shoes, from everyday comfort to statement heels.'
            }
          </p>
        </div>
